...
function setupKeyboard() {
  ...
  window.onkeydown = e => {
    ...
    if (window._debugMode && e.shiftKey) {
      ...
      // Shift+W — spawn storm debug panel toggle
      if (e.code === 'KeyW' && gs && !gs.over) {
        e.preventDefault();
        _toggleStormDebugPanel(gs);
        return;
      }
      // Storm spawn shortcuts (when storm panel is open)
      if (window._stormDebugOpen && gs && !gs.over) {
        const spawnKey = _STORM_DEBUG_KEYS[e.code];
        if (spawnKey) {
          e.preventDefault();
          _debugSpawnStorm(gs, spawnKey);
          return;
        }
      }
    }
...
// ── Storm Debug Panel ──────────────────────────────────────────────────────
// Shift+W toggles; storm keys spawn directly at arena centre when open.

const _STORM_DEBUG_KEYS = {
  // Base storms
  'Digit1': 'HEATWAVE',    'Digit2': 'BLIZZARD',
  'Digit3': 'THUNDERSTORM','Digit4': 'DOWNPOUR',
  'Digit5': 'SANDSTORM',   'Digit6': 'BLACKHOLE',
  // Standard combos (fire-based)
  'KeyQ': 'HEATWAVE_THUNDERSTORM', // Plasma Storm
  'KeyE': 'HEATWAVE_SANDSTORM',    // Firestorm
  'KeyA': 'HEATWAVE_BLIZZARD',     // Flashpoint
  'KeyZ': 'HEATWAVE_DOWNPOUR',     // Magma Surge
  // Ice combos
  'KeyC': 'THUNDERSTORM_BLIZZARD', // Whiteout
  'KeyV': 'SANDSTORM_BLIZZARD',    // Arctic Gale
  'KeyX': 'BLIZZARD_DOWNPOUR',     // Permafrost
  // Lightning / wind / water combos
  'KeyG': 'THUNDERSTORM_SANDSTORM',// Supercell
  'KeyH': 'THUNDERSTORM_DOWNPOUR', // Seismic Charge
  'KeyN': 'SANDSTORM_DOWNPOUR',    // Dust Devil
  // BH combos
  'KeyI': 'BLACKHOLE_HEATWAVE',    // Event Horizon
  'KeyO': 'BLACKHOLE_BLIZZARD',    // Void Frost
  'KeyP': 'BLACKHOLE_THUNDERSTORM',// Dark Matter
  'KeyK': 'BLACKHOLE_DOWNPOUR',    // Abyssal Tide
  'KeyL': 'BLACKHOLE_SANDSTORM',   // Null Vortex
};

window._stormDebugOpen = false;

function _toggleStormDebugPanel(gs) {
  window._stormDebugOpen = !window._stormDebugOpen;
  let panel = document.getElementById('storm-debug-panel');
  if (!panel) {
    panel = document.createElement('div');
    panel.id = 'storm-debug-panel';
    panel.style.cssText = `
      position:fixed; top:48px; right:10px; z-index:99998;
      background:rgba(0,8,4,0.93); border:1px solid #44ffcc;
      border-radius:6px; padding:10px 14px 10px 10px;
      font-family:'Orbitron',monospace; font-size:10px;
      color:#aaffcc; line-height:1.7; pointer-events:none;
      min-width:230px; box-shadow:0 0 20px rgba(0,200,140,0.2);
    `;
    document.body.appendChild(panel);
  }
  if (!window._stormDebugOpen) { panel.style.display = 'none'; return; }
  panel.style.display = 'block';

  // Build label map from COMBO_STORMS + WEATHER_TYPES
  const rows = [
    { header: '── BASE STORMS ──', color: '#44ffcc' },
    { key:'1', id:'HEATWAVE',     color:'#ff6622' },
    { key:'2', id:'BLIZZARD',     color:'#88ddff' },
    { key:'3', id:'THUNDERSTORM', color:'#aa88ff' },
    { key:'4', id:'DOWNPOUR',     color:'#4499ff' },
    { key:'5', id:'SANDSTORM',    color:'#ddaa44' },
    { key:'6', id:'BLACKHOLE',    color:'#9933cc' },
    { header: '── COMBOS ──', color: '#44ffcc' },
    { key:'Q', id:'HEATWAVE_THUNDERSTORM', label:'Plasma Storm',   color:'#ff9900' },
    { key:'E', id:'HEATWAVE_SANDSTORM',    label:'Firestorm',      color:'#ff5500' },
    { key:'A', id:'HEATWAVE_BLIZZARD',     label:'Flashpoint',     color:'#ff44cc' },
    { key:'Z', id:'HEATWAVE_DOWNPOUR',     label:'Magma Surge',    color:'#ff6600' },
    { key:'C', id:'THUNDERSTORM_BLIZZARD', label:'Whiteout',       color:'#88ffee' },
    { key:'V', id:'SANDSTORM_BLIZZARD',    label:'Arctic Gale',    color:'#44eeff' },
    { key:'X', id:'BLIZZARD_DOWNPOUR',     label:'Permafrost',     color:'#88ccff' },
    { key:'G', id:'THUNDERSTORM_SANDSTORM',label:'Supercell',      color:'#aaddff' },
    { key:'H', id:'THUNDERSTORM_DOWNPOUR', label:'Seismic Charge', color:'#bb88ff' },
    { key:'N', id:'SANDSTORM_DOWNPOUR',    label:'Dust Devil',     color:'#ddbb44' },
    { header: '── BLACK HOLE COMBOS ──', color: '#cc88ff' },
    { key:'I', id:'BLACKHOLE_HEATWAVE',    label:'Event Horizon',  color:'#ff6600' },
    { key:'O', id:'BLACKHOLE_BLIZZARD',    label:'Void Frost',     color:'#4488cc' },
    { key:'P', id:'BLACKHOLE_THUNDERSTORM',label:'Dark Matter',    color:'#9933ff' },
    { key:'K', id:'BLACKHOLE_DOWNPOUR',    label:'Abyssal Tide',   color:'#0077aa' },
    { key:'L', id:'BLACKHOLE_SANDSTORM',   label:'Null Vortex',    color:'#cc9900' },
  ];

  let html = `<div style="color:#44ffcc;font-size:11px;font-weight:900;margin-bottom:6px;letter-spacing:1px;">🌩 STORM SPAWNER <span style="color:#556655;font-size:9px;">[Shift+W close]</span></div>`;
  for (const r of rows) {
    if (r.header) {
      html += `<div style="color:#556655;margin:4px 0 2px;letter-spacing:1px;font-size:9px;">${r.header}</div>`;
    } else {
      const name = r.label ?? r.id;
      html += `<div><span style="display:inline-block;width:18px;text-align:center;background:rgba(0,200,140,0.15);border:1px solid #44ffcc55;border-radius:3px;color:#44ffcc;font-size:9px;margin-right:5px;">${r.key}</span><span style="color:${r.color}">${name}</span></div>`;
    }
  }
  html += `<div style="color:#334433;margin-top:6px;font-size:9px;">Hold Shift + key to spawn at centre</div>`;
  panel.innerHTML = html;

  // Flash hint in game
  const p = gs?.players?.[0] ?? gs?.player;
  if (p) spawnFloat(p.x, p.y - 80, '🌩 STORM DEBUG OPEN', '#44ffcc', { char: p, size: 16, life: 2.0 });
}

function _debugSpawnStorm(gs, stormId) {
  if (!gs || !gs.weatherZones) return;
  const ab = getArenaBounds(gs);
  const cx = ab.x + ab.w * (0.35 + Math.random() * 0.30);
  const cy = ab.y + ab.h * (0.35 + Math.random() * 0.30);
  const R  = 300;
  const isCombo = stormId.includes('_');

  if (isCombo) {
    const comboDef = COMBO_STORMS[stormId];
    if (!comboDef) return;
    if (!gs._noiseSeed) gs._noiseSeed = Math.random() * 100;
    const zone = {
      type: stormId, comboKey: stormId, comboDef,
      converged: true,
      x: cx, y: cy, radius: R,
      vx: 0, vy: 0,
      intensity: 1, fadeIn: 0,
      lifetime: 999, fadeOut: 4, age: 0.1,
      announced: true,
    ...
    gs.weatherZones.push(zone);
    spawnFloat(cx, cy - R * 0.7, `[DEBUG] ${comboDef.label}`, comboDef.color, { size: 20, life: 2.5 });
  } else {
    const def = WEATHER_TYPES[stormId];
    if (!def) return;
    const zone = {
      type: stormId, x: cx, y: cy, radius: R,
      vx: 0, vy: 0,
      intensity: 1, fadeIn: 0,
      lifetime: 999, fadeOut: 4, age: 0.1,
      announced: true,
    ...
    gs.weatherZones.push(zone);
    spawnFloat(cx, cy - R * 0.7, `[DEBUG] ${def.label ?? stormId}`, def.color, { size: 20, life: 2.5 });
  }
}
