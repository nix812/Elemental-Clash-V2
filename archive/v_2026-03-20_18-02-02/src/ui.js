  ...
  function buildPatchNotesTab(container) {
    const notes = [
      ...
      },
      {
        ...
        changes: [
          { tag: 'FIX', text: 'Hero grid no longer rebuilds when tapping a slot card or toggling CPU/HUMAN — neither action changes grid contents. Grid now only rebuilds when strip row count crosses the 3→4 player threshold.' },
          { tag: 'FIX', text: 'Player count controls moved to topbar right side so they never overlap hero cards at any player count.' },
        ]
      },
      {
...
function buildLobby() {
  ...
  if (stripEl) {
    stripEl.innerHTML = '';
    const prevRows = document.documentElement.style.getPropertyValue('--strip-rows') || '1';
    ...
    updateLayoutVars();
    // Only rebuild the hero grid when row count changes — avoids flicker on every player add
    const _rowsChanged = useRows !== prevRows;
    ...
    if (countRow) {
      ...
      if (minusBtn) {
        ...
        minusBtn.onclick = (e) => {
          e.stopPropagation();
          if (lobbySlots.length <= 2) return;
          const prevRows = lobbySlots.length >= 4 ? '2' : '1';
          lobbySlots.pop();
          if (activeSlotIdx >= lobbySlots.length) activeSlotIdx = 0;
          const newRows = lobbySlots.length >= 4 ? '2' : '1';
          buildLobby();
          if (newRows !== prevRows) {
      ...
      if (plusBtn) {
        ...
        plusBtn.onclick = (e) => {
          ...
          const newRows = lobbySlots.length >= 4 ? '2' : '1';
          buildLobby();
          // Only rebuild hero grid if strip row count changed — avoids flash
          if (newRows !== prevRows) {
            requestAnimationFrame(() => requestAnimationFrame(() => {
              updateLayoutVars();
              buildHeroGrid('hero-grid','hero-detail');
