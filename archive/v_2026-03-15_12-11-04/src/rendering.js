...
function drawHUD(gs) {
  ...
  // ── Score overlay — below match timer ──
  if (gs.countdown <= 0) {
    const timerSize  = Math.max(11, Math.round(H * 0.022));
    const timerH     = timerSize * 1.3;
    const scoreY     = pad + timerH + Math.round(H * 0.006);
    ...
    const maxKills   = gs.maxKills;

    // Flash the score briefly on a kill event
    if (!gs._scoreFlash) gs._scoreFlash = 0;
    const prevTotal = gs._scorePrevTotal ?? 0;
    const curTotal  = myKills + oppKills;
    if (curTotal > prevTotal) { gs._scoreFlash = 0.7; }
    gs._scorePrevTotal = curTotal;
    if (gs._scoreFlash > 0) gs._scoreFlash = Math.max(0, gs._scoreFlash - 0.03);

    const flashBoost = gs._scoreFlash;
    const baseAlpha  = 0.72 + flashBoost * 0.28;

    // Score sizes
    const scoreSize  = Math.max(13, Math.round(H * 0.028));
    const sepSize    = Math.max(10, Math.round(H * 0.020));
    const kdaSize    = Math.max(8,  Math.round(H * 0.013));

    // Background pill
    const pillW = Math.round(W * 0.18);
    const pillH = Math.round(scoreSize * 1.55 + kdaSize * 1.6 + 6);
    const pillX = cx - pillW / 2;
    ctx.save();
    ctx.globalAlpha = 0.48 + flashBoost * 0.2;
    ctx.fillStyle = 'rgba(0,0,0,0.6)';
    ctx.beginPath(); roundRect(pillX, scoreY, pillW, pillH, Math.round(pillH * 0.28)); ctx.fill();
    ctx.restore();

    // Kill progress bars — thin strips at top of pill
    const barStripH = Math.round(H * 0.004);
    const barStripY = scoreY + Math.round(pillH * 0.07);
    const barPad    = Math.round(pillW * 0.06);
    const barTotalW = pillW - barPad * 2;
    const myFrac    = Math.min(1, myKills  / maxKills);
    const oppFrac   = Math.min(1, oppKills / maxKills);
    // My team bar (left, cyan)
    ctx.globalAlpha = baseAlpha * 0.8;
    ctx.fillStyle = 'rgba(0,0,0,0.4)';
    ctx.beginPath(); roundRect(pillX + barPad, barStripY, barTotalW / 2 - 2, barStripH, barStripH/2); ctx.fill();
    ctx.fillStyle = '#00d4ff';
    ctx.beginPath(); roundRect(pillX + barPad, barStripY, Math.max(barStripH, (barTotalW / 2 - 2) * myFrac), barStripH, barStripH/2); ctx.fill();
    // Opp team bar (right, red) — grows left-to-right mirrored
    ctx.fillStyle = 'rgba(0,0,0,0.4)';
    ctx.beginPath(); roundRect(cx + 2, barStripY, barTotalW / 2 - 2, barStripH, barStripH/2); ctx.fill();
    ctx.fillStyle = '#ff4466';
    ctx.beginPath(); roundRect(cx + 2, barStripY, Math.max(barStripH, (barTotalW / 2 - 2) * oppFrac), barStripH, barStripH/2); ctx.fill();
    ctx.globalAlpha = 1;

    // Kill counts
    const numY = barStripY + barStripH + Math.round(scoreSize * 0.25) + scoreSize;
    ctx.textBaseline = 'alphabetic';
    ctx.lineWidth = 2.5;

    // My score (left of center)
    ctx.font = `900 ${scoreSize}px "Orbitron",monospace`;
    ctx.textAlign = 'right';
    ctx.globalAlpha = baseAlpha;
    ctx.fillStyle = myKills > oppKills ? '#00ffcc' : 'rgba(255,255,255,0.9)';
    ctx.strokeStyle = 'rgba(0,0,0,0.7)'; ctx.lineWidth = 3;
    ctx.strokeText(myKills, cx - Math.round(W * 0.022), numY);
    ctx.fillText(myKills, cx - Math.round(W * 0.022), numY);

    // Separator dash
    ctx.font = `400 ${sepSize}px "Orbitron",monospace`;
    ctx.textAlign = 'center';
    ctx.fillStyle = 'rgba(255,255,255,0.4)';
    ctx.strokeStyle = 'rgba(0,0,0,0.5)'; ctx.lineWidth = 2;
    ctx.strokeText('—', cx, numY);
    ctx.fillText('—', cx, numY);

    // Opp score (right of center)
    ctx.font = `900 ${scoreSize}px "Orbitron",monospace`;
    ctx.textAlign = 'left';
    ctx.fillStyle = oppKills > myKills ? '#ff6688' : 'rgba(255,255,255,0.7)';
    ctx.strokeStyle = 'rgba(0,0,0,0.7)'; ctx.lineWidth = 3;
    ctx.strokeText(oppKills, cx + Math.round(W * 0.022), numY);
    ctx.fillText(oppKills, cx + Math.round(W * 0.022), numY);

    // K/D/A row
    const p       = gs.player;
    const kdaY    = numY + Math.round(kdaSize * 1.5);
    const kdaStr  = `${p.kills||0}K  ${p.deaths||0}D  ${p.assists||0}A`;
    ctx.font = `700 ${kdaSize}px "Orbitron",monospace`;
    ctx.textAlign = 'center';
    ctx.fillStyle = `rgba(180,210,255,${0.6 * baseAlpha})`;
    ctx.strokeStyle = 'rgba(0,0,0,0.5)'; ctx.lineWidth = 2;
    ctx.strokeText(kdaStr, cx, kdaY);
    ctx.fillText(kdaStr, cx, kdaY);
