...
const Audio = (() => {
  ...
  function init() {
    ...
    startHum();
    // Decode any raw buffers that arrived before AudioContext existed
    if (window._menuBgmRaw) { loadMenuBGM(window._menuBgmRaw); window._menuBgmRaw = null; }
    if (window._matchBgmRaw) { loadMatchBGM(window._matchBgmRaw); window._matchBgmRaw = null; }
    // Legacy path — buffer arrived via old window._bgmBuffer route
    if (!matchBuffer && window._bgmBuffer) loadMatchBGM(window._bgmBuffer);
    if (!menuBuffer && window._menuBgmBuffer) loadMenuBGM(window._menuBgmBuffer);
    // If we're already on a menu screen, queue menu BGM
    const active = document.querySelector('.screen.active');
    if (active && ['menu','hero-select','how-to-play','options'].includes(active.id)) {
      _pendingMenuPlay = true;
    }
  }
  ...
  function loadMenuBGM(arrayBuffer) {
    if (!ctx) {
      // AudioContext not ready yet — store raw buffer and decode when init() runs
      window._menuBgmRaw = arrayBuffer;
      return;
    }
  ...
  function loadMatchBGM(arrayBuffer) {
    if (!ctx) {
      // AudioContext not ready yet — store raw buffer and decode when init() runs
      window._matchBgmRaw = arrayBuffer;
      return;
    }
    ctx.decodeAudioData(arrayBuffer.slice(0), buf => {
      matchBuffer = buf;
  ...
  function _playTrack(buffer, trackId) {
    ...
    _stopTrack(0.5);
    if (!_onFor(trackId)) return; // respect per-track toggle
    const src = ctx.createBufferSource();
    const g   = ctx.createGain();
    src.buffer = buffer;
    src.loop   = true;
    g.gain.setValueAtTime(0, ctx.currentTime);
    g.gain.linearRampToValueAtTime(_volFor(trackId), ctx.currentTime + 0.8);
  ...
  function setMenuMusicVol(v) {
    ...
    if (activeTrack === 'menu' && activeGainNode && settings.menuMusicOn)
      activeGainNode.gain.linearRampToValueAtTime(v, ctx.currentTime + 0.05);
  }
  function setMatchMusicVol(v) {
    settings.matchMusicVol = v; save();
    if (activeTrack === 'match' && activeGainNode && settings.matchMusicOn)
      activeGainNode.gain.linearRampToValueAtTime(v, ctx.currentTime + 0.05);
  }
  function setMenuMusicOn(on) {
    ...
    if (!ctx) return;
    if (activeTrack === 'menu') {
      if (on) activeGainNode?.gain.linearRampToValueAtTime(settings.menuMusicVol, ctx.currentTime + 0.4);
      else    { activeGainNode?.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.4); }
    } else if (on && !activeTrack) {
      playMenuBGM();
    }
  }
  function setMatchMusicOn(on) {
    ...
    if (!ctx) return;
    if (activeTrack === 'match') {
      if (on) activeGainNode?.gain.linearRampToValueAtTime(settings.matchMusicVol, ctx.currentTime + 0.4);
      else    activeGainNode?.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.4);
    }
  }
