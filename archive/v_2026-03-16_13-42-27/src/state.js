...
function applyWeatherToChar(c, gs, dt) {
  ...
  if (c.weatherBlackholePull) {
    ...
    const normX = dx / dist;
    const normY = dy / dist;
    // depth: 0 at edge, 1 at dead centre
    const depth = Math.max(0, 1 - dist / zones[0].zone.radius);

    const isSprinting = (c.sprintTimer ?? 0) > 0;

    if (isSprinting) {
      // Sprint: instant position push outward — no velocity, no bounce, clean escape
      const escapeDist = 5.5;
      c.x -= normX * escapeDist;
      c.y -= normY * escapeDist;
      // Also zero out any inward velocity so you don't get sucked back immediately
      const inwardComponent = (c.velX || 0) * normX + (c.velY || 0) * normY;
      if (inwardComponent > 0) {
        c.velX = (c.velX || 0) - normX * inwardComponent;
        c.velY = (c.velY || 0) - normY * inwardComponent;
      }
    } else {
      // Pull: direct position nudge toward centre each frame — no velocity accumulation,
      // no bounce. Ramps from gentle at edge to strong at centre.
      // At edge (depth~0): ~0.3 px/frame — barely noticeable
      // At centre (depth~1): ~4.5 px/frame — hard to fight but not impossible
      const pullPx = (0.3 + depth * depth * 4.2) * (vp.force / 200);
      c.x += normX * pullPx;
      c.y += normY * pullPx;

      // Movement speed penalty: full speed at edge, 40% speed at centre
      // Applied as a multiplier on the hero's base speed this frame only
      const speedMult = 1.0 - depth * depth * 0.60;
      if (!c._bhSpeedMult || c._bhSpeedMult > speedMult) {
        c._bhSpeedMult = speedMult; // picked up by movement code this frame
      }

      // Also drain any outward velocity the player has built up —
      // you can still walk out, it just takes sustained effort
      const outwardComponent = (c.velX || 0) * (-normX) + (c.velY || 0) * (-normY);
      if (outwardComponent > 0) {
        const drain = Math.min(outwardComponent, depth * depth * outwardComponent * 0.5);
        c.velX = (c.velX || 0) + normX * drain;
        c.velY = (c.velY || 0) + normY * drain;
      }
    ...
    // Reset reaction timer when bot is sprinting out
    if (!c.isPlayer && isSprinting) c._bhReactTimer = undefined;
  } else {
    // Outside zone — clear speed mult and bot timer
